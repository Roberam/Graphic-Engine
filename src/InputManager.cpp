...
bool InputManager::Init()
{
	...
	if (GetSystemMetrics(SM_MOUSEPRESENT))
		bool prueba = true;
	CreateVirtualButton("ARRIBA",Key_W);
	CreateVirtualButton("ABAJO",Key_S);
	CreateVirtualButton("IZQUIERDA",Key_A);
	CreateVirtualButton("DERECHA",Key_D);
	CreateVirtualAxis("EJEX", Mouse_Right, Mouse_Left);
	CreateVirtualAxis("EJEY", Mouse_Up, Mouse_Down);
...
void InputManager::Update()
{
	...
	for (int i=0; i<_myVirtualButtons.Size();i++)
	{
	...
	for (int i=0; i<_myVirtualAxis.Size();i++)
	{
...
{
	int i=0;
	return (existsVButton(i,name)
			&& _myVirtualButtons[i].keyPress);
}
...
bool InputManager::IsKeyDown( eInputCode vkCode ){return false;}

bool InputManager::IsKeyUp( eInputCode vkCode ){return false;}

bool InputManager::IsMouseButtonPressed( eInputCode button ){return false;}

bool InputManager::GetMouseButtonDown( eInputCode button ){return false;}

bool InputManager::GetMouseButtonUp( eInputCode button ){return false;}
...
bool InputManager::existsVButton(int& index, const String name) const
{
	...
	if (_myVirtualButtons.Size()>0)
		while ((!found) && (index<_myVirtualButtons.Size()))
		{
...
bool InputManager::existsVAxis(int& index, const String name) const
{
	...
	if (_myVirtualAxis.Size()>0)
		while ((!found) && (index<_myVirtualAxis.Size()))
		{
